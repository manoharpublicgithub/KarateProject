Feature: Kafka Test Suite3
  ...
  Scenario: Creating Consumer
    ...
    Then status 200
    * print response
    * match response.instance_id == "Manohar_consumer"
  ...
  Scenario: Subscribing to topic
    Given path "/consumers/my-consumer-group/instances/Manohar_consumer/subscription"
    And request
      """
      {
        "topics" : [ "manohar" ]
  ...
  Scenario: Getting Subscription
    ...
    Then status 200
    * print response
    * assert response.topics[0] == "manohar"


  Scenario: Getting Subscription records
    Given path "/consumers/my-consumer-group/instances/Manohar_consumer/records"
  ...
  Scenario: Getting assigned Assignments
    Given path "/consumers/my-consumer-group/instances/Manohar_consumer/assignments"
    And header Accept = "application/vnd.kafka.v2+json"
    When method get
    Then status 200
    * print response
    * assert response.partitions[0].topic == "manohar"
  ...
  Scenario: Deleting consumer
    Given path "/consumers/my-consumer-group/instances/Manohar_consumer"
    And header Accept = "application/vnd.kafka.v2+json"
    When method delete
    Then status 204
